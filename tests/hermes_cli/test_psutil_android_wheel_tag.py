...
def test_android_wheel_platform_ignores_runtime_android_api(monkeypatch):
    ...
    monkeypatch.delenv("HERMES_ANDROID_API_LEVEL", raising=False)

    assert android_wheel_platform_tag(machine="aarch64") == "android_24_arm64_v8a"


def test_android_wheel_platform_supports_explicit_build_target(monkeypatch):
    monkeypatch.setenv("HERMES_ANDROID_API_LEVEL", "28")

    assert android_wheel_platform_tag(machine="aarch64") == "android_28_arm64_v8a"


def test_android_wheel_platform_rejects_unknown_architecture():
    with pytest.raises(PsutilAndroidInstallError, match="Unsupported Android architecture"):
