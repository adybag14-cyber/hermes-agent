...
def test_android_package_root_preserves_loaded_dependency_utils(monkeypatch):
    import sys
    import types
    ...
    assert shared.is_truthy_value("yes") is True
    assert callable(shared.atomic_replace)
...
def test_provider_setup_webview_errors_show_browser_copy_fallback():
    ...
    assert 'listOf("openrouter", "alibaba", "alibaba-coding-plan", "qwen-oauth", "zai")' in activity_test
