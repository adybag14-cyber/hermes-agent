...
def _chatgpt_web_browser_pages(debug_base: str) -> list[dict[str, Any]]:
    with urllib.request.urlopen(f"{debug_base}/json/list", timeout=5) as response:
        payload = json.load(response)
    return [item for item in payload if isinstance(item, dict)] if isinstance(payload, list) else []


def _is_chatgpt_browser_page(item: dict[str, Any]) -> bool:
    try:
        parsed = urllib.parse.urlsplit(str(item.get("url") or ""))
        return item.get("type") == "page" and parsed.scheme == "https" and parsed.hostname == "chatgpt.com"
    except ValueError:
        return False
...
async def _chatgpt_web_browser_fetch(
    *,
    ...
    if page is None:
        ...
        page = await asyncio.to_thread(_chatgpt_web_browser_page_target, debug_base, created_target_id)
...
async def _chatgpt_web_browser_create_target(debug_base: str, url: str) -> str:
    version = await asyncio.to_thread(_chatgpt_web_browser_version, debug_base)
...
async def _chatgpt_web_browser_multimodal_completion(
    *,
    ...
    for source in image_sources:
        materialized_path, cleanup_path = await asyncio.to_thread(_materialize_chatgpt_web_browser_image, source)
    ...
    try:
        ...
        while time.monotonic() < deadline:
            try:
                page = await asyncio.to_thread(_chatgpt_web_browser_page_target, debug_base, target_id)
            except Exception:
                page = None
...
def stream_chatgpt_web_completion(
    *,
    ...
    if credential_snapshot is not None:
        from agent.chatgpt_credentials import validate_chatgpt_web_credentials

        snapshot = validate_chatgpt_web_credentials(credential_snapshot, token)
        session_token = snapshot.session_token
        cookie_header = snapshot.cookie_header
        browser_cookies = snapshot.browser_cookies
        # A selected grant with blank metadata must not be refilled from an
        # environment or another pool identity when the transport is invoked.
        ua = snapshot.user_agent or DEFAULT_CHATGPT_WEB_USER_AGENT
        did = snapshot.device_id or _extract_cookie_value(cookie_header, "oai-did") or str(uuid.uuid4())
    else:
