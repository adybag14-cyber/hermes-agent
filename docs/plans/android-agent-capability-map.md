...
| Tasker plugin model | Present for a safe action-plugin slice. Hermes exposes a Locale-compatible Tasker edit activity and fire receiver so Tasker can run an existing saved Hermes automation by id. Each plugin configuration stores a per-action token in Hermes preferences and the fire receiver refuses missing or mismatched tokens, so arbitrary third-party broadcasts cannot run automations by guessing an id. Full Tasker condition/event plugin profiles, variable-return plugin APIs, and plugin-host timeout/status integration are not yet present. |
