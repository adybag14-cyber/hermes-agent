...
| Tasker-style manual tasks/actions | Present for saved shell, file-write, file-delete, and safe Android system-action records through `android_automation_tool`; chat-triggered terminal, file, UI, and Android system tool calls remain available. |
...
| Tasker-style variables | Present as a durable Android automation variable table exposed through `android_automation_tool`; shell commands, file paths, file content, and system-action names can expand `%NAME` or `{{NAME}}` values at run time. |
...
The next credible Tasker-parity feature after shell/file/system actions,
variables, and basic phone-state triggers should extend profile coverage with
explicit user-visible records:

- UI action records beyond shell commands,
