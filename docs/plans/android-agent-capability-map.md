...
- Shizuku is an Android app plus API that lets normal apps use selected Android
  ...
  system services through a server process started with ADB or root.
- Shizuku-API requires the user to install Shizuku or Sui first. Android 11+
  ...
  checks, remote binder calls, and user services.
...
- Tinker by Shopify is a closed-source mobile AI creative-workflow app. Public
  ...
  ad, project, sharing, remixing, and tool-chaining workflows.
...
| Accessibility-based UI snapshot/action | Present through `HermesAccessibilityUiBridge` when the user enables the accessibility service. |
...
| Emulator/BlueStacks visual validation | Added host harness: `scripts/android_visual_harness.py` for ADB screenshots, taps, swipes, text input, launch, and wide resolution settings. |
