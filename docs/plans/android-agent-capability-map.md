...
| Tasker-style manual tasks/actions | Present for saved shell, file-write, file-delete, safe Android system-action, accessibility UI-action, app-launch, Android intent, notification post/cancel/button, launcher shortcut, and Shizuku/Sui package-permission records through `android_automation_tool`; chat-triggered terminal, file, UI, and Android system tool calls remain available. Saved intent records cover explicit activity start, URI open, and package-targeted broadcast intents. Notification actions can post/update/cancel Hermes app notifications with title, text, channel, priority, group, ongoing, auto-cancel, only-alert-once, group-summary fields, and up to three safe action buttons that open Hermes, dismiss the notification, or run a saved Hermes automation by id; Android 13+ devices must grant notification permission first. Launcher shortcut actions can create, list, or remove Android launcher shortcuts for saved Hermes automations; shortcut taps open Hermes and run the selected automation. |
...
| Tasker-style scenes/widgets | Partially present. Hermes can create Android launcher shortcuts for saved automations, but user-created Android scenes, overlays, and home-screen widgets are not yet present. Hermes has its fixed app UI and accessibility control of other apps. |
...
saved Android intent actions, saved notification actions, launcher shortcuts, saved Shizuku
