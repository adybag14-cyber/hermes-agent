        ...
        return JSONObject()
            ...
            .put(
                ...
                "You are Hermes running inside the native Android app. " +
                    ...
                    "When the user asks to create a recurring phone automation, reusable Android task, Tasker-like variable, time/day trigger, phone-state trigger, app-foreground trigger, notification trigger, calendar event trigger, location trigger, sensor trigger, Shizuku availability trigger, saved file action, safe saved Android settings action, saved visible-UI action, saved Android intent, broadcast, URI, activity launch, saved app-launch action, or backup/restore Hermes automations, call android_automation_tool. It can save shell, file-write, file-delete, safe Android system-action, accessibility UI-action, app-launch, Android intent, and Shizuku package-permission tasks, export/import Hermes automation bundles, run tasks manually, enable/disable/delete them, schedule interval and time-of-day tasks with Android alarms, run boot/power/battery/time/app-foreground/notification-posted/calendar-event/location/sensor/Shizuku-state triggers, and expand saved variables in commands, file content, UI selectors, intent fields, package names, trigger packages, Shizuku state fields, and notification, calendar, location, sensor, or time event fields. " +
                                    ...
                                    .put(
                                        ...
                                        JSONObject()
                                            ...
                                            .put(
                                                ...
                                                JSONObject()
                                                    .put("type", "string")
                                                    .put("description", "Optional trigger for create_*_task or run_trigger: manual, time, boot, power_connected, power_disconnected, battery_low, battery_okay, app_foreground, notification_posted, calendar_event, location, sensor, shizuku_available, or shizuku_unavailable. interval_minutes creates an interval trigger. time requires a time argument such as 08:30 and can use days_of_week. app_foreground and notification_posted require trigger_package_name and the relevant Android service permission. calendar_event can filter by calendar_name, title_contains, description_contains, or location_contains. location can filter by latitude, longitude, radius_meters, location_provider, location_name, or max_accuracy_meters and must be run with run_location_trigger. sensor can filter by sensor_type, sensor_event, value_name, min_value, or max_value and must be run with run_sensor_trigger."),
                                            )
                                            .put(
