...
enum class McpRuntimeText(
    ...
    STDIO_HELP(
        "Stdio needs an Android-compatible executable already installed on the device. Add arguments in Advanced JSON. Agent does not install Python, Node or server packages automatically.",
        "Stdio 需要设备上已安装且兼容 Android 的可执行程序。请在高级 JSON 中添加参数。Agent 不会自动安装 Python、Node 或服务器软件包。",
        "Stdio necesita un ejecutable compatible con Android ya instalado. Añade argumentos en JSON avanzado. Agent no instala Python, Node ni paquetes de servidor automáticamente.",
        "Stdio benötigt ein bereits installiertes Android-kompatibles Programm. Argumente im erweiterten JSON hinzufügen. Agent installiert Python, Node oder Serverpakete nicht automatisch.",
        "Stdio exige um executável compatível com Android já instalado. Adicione argumentos no JSON avançado. O Agent não instala Python, Node ou pacotes de servidor automaticamente.",
        "Stdio nécessite un exécutable compatible Android déjà installé. Ajoutez les arguments dans le JSON avancé. Agent n'installe pas automatiquement Python, Node ou les paquets serveur.",
    ),
    ...
    RESTART("MCP cleanup was not verified. Force stop and reopen Agent before trying again.", "无法验证 MCP 是否已完全停止。请强行停止并重新打开 Agent 后再试。", "No se verificó el cierre de MCP. Fuerza el cierre de Agent y vuelve a abrirlo antes de reintentar.", "MCP-Bereinigung nicht bestätigt. Agent zwangsweise beenden und vor einem neuen Versuch öffnen.", "O encerramento do MCP não foi verificado. Force a parada e reabra o Agent antes de tentar novamente.", "L'arrêt de MCP n'a pas été vérifié. Forcez l'arrêt de Agent et rouvrez-le avant de réessayer."),
