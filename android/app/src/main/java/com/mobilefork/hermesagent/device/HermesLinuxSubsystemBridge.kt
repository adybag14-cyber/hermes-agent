...
object HermesLinuxSubsystemBridge {
    ...
    private fun invalidateRuntimeState(context: Context, reason: String) {
        ...
        Log.w(TAG, "Invalidated incompatible Agent runtime state without deleting user sandboxes: $reason")
    }
    ...
    private fun writeNativeCommandEnvironment(
        ...
        val content = buildString {
            append("# Generated by Agent. Source this file; do not execute it.\n")
    ...
    fun buildRunEnvironment(state: JSONObject): Map<String, String> {
        ...
        return mapOf(
            ...
            "TERMUX_APP__APP_VERSION_NAME" to "Agent",
            "TERMUX_VERSION" to "Agent",
    ...
    fun commandWithEmbeddedToolAliases(state: JSONObject, command: String): String {
        ...
        val prelude = listOf(
            ...
            "export TERMUX_APP__APP_VERSION_NAME=Agent",
            "export TERMUX_VERSION=Agent",
            ...
            "pkg() { echo \"Agent host packages are supplied only by a signed APK; use linux_host_pkg_tool for status/list/search.\" >&2; echo \"For guest Debian/Alpine updates use linux_sandbox_tool action=update.\" >&2; return 64; }",
            "hermes-pkg() { pkg \"\$@\"; }",
            "apt() { echo \"Host packages require a signed Agent APK. For guest distro packages use linux_sandbox_tool action=update.\" >&2; return 64; }",
            "apt-get() { apt \"\$@\"; }",
        ).joinToString("; ")
